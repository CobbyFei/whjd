	...
	$(function() {
		...
		datagrid = $('#datagrid')
				.datagrid(
						{
							url : sy.bp()
									+ '/freeAccelerationMethodAction!getAllMethodDetections.action',
							title : '委托单基本信息',
							iconCls : 'icon-save',
							pagination : true,
							pagePosition : 'bottom',
							...
							function(r) {
								if (r) {
									...
									$
											.ajax({
												url : sy.bp()
														+ '/freeAccelerationMethodAction!batchConclusion.action',
												data : {
													ids : ids.join(',')
												},
												dataType : 'json',
		...
		if (rows.length == 1) {
			...
			var p = parent.sy
					.dialog({
						title : '打印检测报告',
						href : sy.bp()
								+ '/freeAccelerationMethodAction!methodPrint.action?id='
								+ rows[0].id,
						width : 1000,
						height : 500,
						resizable : true,
						buttons : [ {
							...
							handler : function() {
								...
								LODOP.ADD_PRINT_HTM(150, "1%", "98%", "98%", f
										.html());
								LODOP.PREVIEW();
							}
						} ]
			...
			$.messager.alert('提示', '一次只能打印一条记录！', 'error');
		} else {
			$.messager.alert('提示', '请选择要打印的记录！', 'error');
		}
	}
	function edit() {
		if (stationName == "市局") {
			...
			if (rows.length == 1) {
				...
				var p = parent.sy
						.dialog({
							title : '编辑检测信息',
							href : sy.bp()
									+ '/freeAccelerationMethodAction!methodEdit.action',
							width : 1000,
							height : 500,
							resizable : true,
							buttons : [
										...
										handler : function() {
											...
											f
													.form(
															...
															{
																url : sy.bp()
																		+ '/freeAccelerationMethodAction!updateMethod.action',
																success : function(
																		d) {
																	var json = $
																			.parseJSON(d);
										...
										handler : function() {
											...
											f
													.form(
															...
															{
																url : sy.bp()
																		+ '/freeAccelerationMethodAction!beginDetection.action',
																success : function(
																		d) {
																	var json = $
																			.parseJSON(d);
								...
								f
										.form(
												'load',
												{
													...
													intHasEmissionProcessDevice : rows[0].intHasEmissionProcessDevice,
													testDeviceModel : rows[0].testDeviceModel,
													temperature : rows[0].temperature,
													airPressure : rows[0].airPressure,
													relativeHumidity : rows[0].relativeHumidity,
													lastOneTest : rows[0].lastOneTest,
													lastTwoTest : rows[0].lastTwoTest,
													lastThreeTest : rows[0].lastThreeTest,
													rpm : rows[0].rpm,
