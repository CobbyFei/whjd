...
<script type="text/javascript">
	$(function() {
		datagrid = $('#datagrid')
				.datagrid(
						{
							url : sy.bp()
									+ '/twoSpeedIdleLimitValueReferenceAction!getLimitValues.action',
							title : '双怠速法限值信息',
							iconCls : 'icon-save',
							pagination : true,
							pagePosition : 'bottom',
	...
	function saveregZJ() {
		$('#frm_ZJ_modify').form(
						...
						{
							url : sy.bp()
									+ '/twoSpeedIdleLimitValueReferenceAction!saveLimitValue.action',
							onSubmit : function() {
					...
					.messagerConfirm(
							...
							function(r) {
								if (r) {
									...
									$.ajax({
												url : sy.bp()
														+ '/twoSpeedIdleLimitValueReferenceAction!deleteLimitValue.action',
												data : {
													ids : ids.join(',')
												},
												dataType : 'json',
		...
		if (rows.length == 1) {
			var p = parent.sy
					.dialog({
						title : '修改限值信息',
						href : sy.bp()
								+ '/twoSpeedIdleLimitValueReferenceAction!limitEdit.action',
						width : 800,
						height : 300,
						resizable : true,
						buttons : [ {
							...
							handler : function() {
								...
								f
										.form(
												...
												{
													url : sy.bp()
															+ '/twoSpeedIdleLimitValueReferenceAction!editLimit.action',
													success : function(d) {
														var json = $
																.parseJSON(d);
														if (json.success) {
...
<body class="easyui-layout" data-options="fit:true">
	<div data-options="region:'north',border:false,title:'添加双怠速法限值'"
